...
// In your database route file (routes/databases.js)
...
// Then modify the POST route to include API key generation and storage
router.post('/', upload.single('csvFile'), async (req, res) => {
  // ... existing code until after database creation ...

  try {
    // ... existing database creation code ...

    // After creating the database and table, generate an API key
    const apiKey = generateApiKey();
    const apiKeyTableQuery = `
    ...
    await dbPool.query(apiKeyTableQuery);
    ...
    console.log('Generated and stored API key for database');
    ...
    res.status(201).json({
      ...
      table: tableName,
      rowsInserted: insertedRows,
      apiKey: apiKey // Send the API key back to the client
    });
