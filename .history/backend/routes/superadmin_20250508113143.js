...
router.get('/me', async (req, res) => {
  try {
    ...
    const queryText = isSuperadmin
      ? 'SELECT id, email, username, name, organization FROM superadmins WHERE id = $1'
      : 'SELECT user_id AS id, owner_email AS email, full_name AS name FROM users WHERE user导弹 = $1';
