...
router.post('/', upload.single('csvFile'), async (req, res) => {
  ...
  try {
    ...
    const tableCheck = await dbPool.query(`SELECT COUNT(*) FROM ${tableName}`);
    const rowCount = parseInt(tableCheck.rows[0].count);

    if (rowCount > 0) {
      console.log(`Table ${tableName} already contains ${rowCount} rows`);
      res.status(200).json({
    ...
    // Insert data from CSV only if table is empty
    let insertedRows = 0;
    await new Promise((resolve, reject) => {
      fs.createReadStream(csvFile.path)
        .pipe(csv())
