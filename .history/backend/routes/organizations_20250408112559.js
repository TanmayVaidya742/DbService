...
// Get all organizations from superadmin table
router.get('/', async (req, res) => {
  ...
  try {
    const result = await pool.query(`
      SELECT DISTINCT organization as organization_name, name as owner_name, email as domain
      FROM superadmins
...
router.post('/', async (req, res) => {
  ...
  try {
    // Check if organization already exists in superadmin table
    const checkResult = await pool.query(
      'SELECT * FROM superadmins WHERE organization = $1',
      [organizationName]
    );
    ...
    if (checkResult.rows.length === 0) {
      return res.status(400).json({
        ...
        message: 'Organization does not exist in superadmin table'
      });
    }
